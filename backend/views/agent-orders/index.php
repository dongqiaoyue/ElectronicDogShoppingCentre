        ...
        <div class="ibox-content">
            ...
            <form class="layui-form"> <!-- 提示：如果你不想用form，你可以换成div等任何一个普通元素 -->
                ...
                <div class="layui-input-item">
                    ...
                    <div class="layui-inline">
                        <?php if(\common\helpers\Tools::authCheck('agent-orders/comselected')): ?>
                    ...
                    <div class="layui-inline" style="margin-left: 10px;">
                        ...
                        <div class="layui-inline ">
                            <?php if(\common\helpers\Tools::authCheck('agent-orders/agent-excel')): ?>
...
<script type="text/javascript">
    ...
    //导出
    function output() {
        window.location.href='<?=\yii\helpers\Url::toRoute("agent-orders/agent-excel")?>'+'?status='+$('#checkStatus').val();
    }

    //发货
    function ordersDeliv(id) {
        layer.open({
            type: 2,
            title: '发货',
            shadeClose: true,
            shade: 0.2,
            area: ['60%', '35%'],
            content: '/agent-orders/deliv?id=' + id
        });
    }
    ...
    function ordersComplete(id){
        layer.confirm('确认完成此订单?', {icon: 3, title:'提示'}, function(index){
            //do something
            $.getJSON("/agent-orders/complete", {'id' : id}, function(res){
                if(1 == res.code){
                    layer.msg(res.msg);
                    setTimeout(function(){
                        initTable();
    ...
    function ordersComSelected() {
        layer.confirm('确认完成所选订单?', {icon: 3, title:'提示'}, function(index) {
            ...
            $.getJSON("/agent-orders/com-selected", {'ids' : ids}, function(res){
                if(1 == res.code){
                    layer.msg(res.msg);
                    setTimeout(function(){
                        initTable();
