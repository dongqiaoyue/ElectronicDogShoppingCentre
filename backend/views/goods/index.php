            ...
            <form class="layui-form"> <!-- 提示：如果你不想用form，你可以换成div等任何一个普通元素 -->
                ...
                <div class="layui-input-item">
                    <div class="layui-inline">
                        <?php if(\common\helpers\Tools::authCheck('goods/create')): ?>
                        <button type="button" id="add" class="btn btn-outline btn-primary ">添加商品</button>
                        <?php endif;?>
                    </div>
                    <div class="layui-inline">
                        <?php if(\common\helpers\Tools::authCheck('goods/del-selected')): ?>
...
<script type="text/javascript">
    ...
    function goodsCheck(id){
        $.getJSON("/goods/check", {'id' : id}, function(res){
            if(1 == res.code){
                layer.msg(res.msg);
                initTable();
    ...
    function goodsEdit(id) {
        layer.open({
            type: 2,
            //title: id,
            title: '<span id="title" style="display: none;">' + id + '</span><span>商品编辑</span>',
            shadeClose: true,
            shade: 0.2,
            area: ['99%', '99%'],
    ...
    function goodsDel(id){
        layer.confirm('确认删除此商品?', {icon: 3, title:'提示'}, function(index){
            //do something
            $.getJSON("/goods/del", {'id' : id}, function(res){
                if(1 == res.code){
                    layer.msg(res.msg);
                    setTimeout(function(){
                        initTable();
    ...
    function goodsDelSelected() {
        layer.confirm('确认删除所选商品?', {icon: 3, title:'提示'}, function(index) {
            ...
            $.getJSON("/goods/del-selected", {'ids' : ids}, function(res){
                if(1 == res.code){
                    layer.msg(res.msg);
                    setTimeout(function(){
                        initTable();
