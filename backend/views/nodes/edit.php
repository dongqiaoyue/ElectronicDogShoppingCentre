...
<div class="ibox-content">
    <form class="form-horizontal m-t" method="post" action="/nodes/edit" id="commentForm">
        ...
        <div class="form-group">
            ...
            <div class="input-group col-sm-7">
                <select class="form-control" name="parentID" required="" aria-required="true">
                    <option value="0" <?php if(0 == $info['parentID']): ?> selected <?php endif; ?>>顶级节点</option>
                    <?php foreach($nodes as $key => $vo): ?>
                        <option value="<?= $vo['ID']; ?>" <?php if($vo['ID'] == $info['parentID']): ?> selected <?php endif; ?>><?= $vo['Name']; ?></option>
                        <?php if(!empty($vo['children'])): ?>
                            <?php foreach($vo['children'] as $k => $v): ?>
                                <option value="<?= $v['ID']; ?>" <?php if($v['ID'] == $info['parentID']): ?> selected <?php endif; ?>>&nbsp;&nbsp;&nbsp; |-- <?= $v['Name']; ?></option>
                            <?php  endforeach; ?>
                        <?php endif; ?>
                    <?php endforeach; ?>
                </select>
        ...
        <div class="form-group">
            ...
            <div class="input-group col-sm-7">
                <input id="auth_rule" type="text" class="form-control" name="auth_rule" placeholder="全小写" value="<?= $info['auth_rule'] ?>">
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="input-group col-sm-7">
                <select class="form-control" name="is_menu" required="" aria-required="true" id="is_menu">
                    <option value="1" <?php if(1 == $info['is_menu']): ?> selected <?php endif; ?> >否</option>
                    <option value="2" <?php if(2 == $info['is_menu']): ?> selected <?php endif; ?>>是</option>
                </select>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="input-group col-sm-7">
                <input id="style" type="text" class="form-control" name="style" placeholder="只有顶级节点需要" value="<?= $info['style'] ?>">
            </div>
        </div>
