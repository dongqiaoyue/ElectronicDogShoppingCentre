...
<div class="ibox-content">
    <form class="form-horizontal m-t" method="post" action="/roles/edit" id="commentForm">
        <input name="_csrf-backend" type="hidden" id="_csrf" value="<?= \Yii::$app->request->csrfToken ?>">
        <input name="role_id" type="hidden" value="<?= $info['role_id']; ?>"/>
        <div class="form-group">
            <label class="col-sm-3 control-label">角色名称：</label>
            <div class="input-group col-sm-7">
                <input id="role_name" type="text" class="form-control" name="role_name" required="" aria-required="true" value="<?= $info['role_name']; ?>">
            </div>
        </div>
            ...
            <div class="input-group col-sm-7">
                <select class="form-control" name="status" required="" aria-required="true" id="status">
                    <?php foreach($status as $key => $vo): ?>
                        <option value="<?= $key ?>" <?php if($key == $info['status']): ?> selected <?php endif; ?>><?= $vo ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>
